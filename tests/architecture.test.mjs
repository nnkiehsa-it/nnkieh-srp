...
test('primary navigation keeps desktop chrome while mobile routes use a source-aware hierarchy', async () => {
  ...
  const issueBoardData = await read('src/composables/useIssueBoardData.ts');
  ...
  assert.doesNotMatch(app, /requestIdleCallback|setTimeout\(preload/u);
  ...
  assert.match(routeComponents, /Promise\.all\(routeNames\.map\(preloadRouteComponent\)\)/u);
  assert.match(routeComponents, /'notifications',[\s\S]*'settings',[\s\S]*'facility-detail'/u);
  ...
  assert.match(app, /:key="viewRoute\.path"/u);
  ...
  assert.match(baseStyles, /\.route-root-enter-active,[\s\S]*opacity 90ms/u);
  assert.match(baseStyles, /\.route-push-enter-active,[\s\S]*opacity 140ms[\s\S]*transform 140ms/u);
  assert.match(baseStyles, /\.route-root-leave-active,[\s\S]*position: absolute/u);
  assert.match(baseStyles, /\.route-push-enter-from \{[\s\S]*translate3d\(10px, 0, 0\)/u);
  assert.match(baseStyles, /\.route-pop-enter-from \{[\s\S]*translate3d\(-6px, 0, 0\)/u);
  ...
  assert.match(hierarchy, /toDepth > fromDepth[\s\S]*'route-push'[\s\S]*toDepth < fromDepth[\s\S]*'route-pop'/u);
  assert.doesNotMatch(issueBoardData, /no-transitions/u);
  assert.doesNotMatch(responsiveStyles, /\.no-transitions/u);
