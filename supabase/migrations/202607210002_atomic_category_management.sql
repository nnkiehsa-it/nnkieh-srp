-- Persist the complete category-management draft in one transaction. This keeps
-- defaults, feature switches, and edits across both domains from partially saving.
...
begin
  ...
  update app_private.issue_categories set is_default = false
  where is_default;
  for category in select value from jsonb_array_elements(issue_categories)
  loop
    select * into existing_issue from app_private.issue_categories
    ...
    insert into app_private.issue_categories as saved(
      ...
      (category->>'commentsEnabled')::boolean,(category->>'isActive')::boolean,
      ...
      comments_enabled=excluded.comments_enabled,is_active=excluded.is_active,
  ...
  update app_private.facility_categories set is_default = false
  where is_default;
  for category in select value from jsonb_array_elements(facility_categories)
  loop
    select * into existing_facility from app_private.facility_categories
    where id = category->>'id';
    ...
    insert into app_private.facility_categories as saved(
      ...
      category->>'id',btrim(category->>'label'),(category->>'isActive')::boolean,
      (category->>'isDefault')::boolean,(category->>'sortOrder')::integer,
      coalesce(existing_facility.created_by,actor_uid),now()
    ) on conflict(id) do update set
      label=excluded.label,is_active=excluded.is_active,is_default=excluded.is_default,
